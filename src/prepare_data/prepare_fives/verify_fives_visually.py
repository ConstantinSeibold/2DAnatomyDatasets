"""
Visual-verify pass for FIVES.

Renders overlays to ./verify_visually/fives/<split>/sample_NN.png and
...
from anatomy_datasets.datasets.fives import FIVES_Dataset
...
def main() -> None:
    root_path = os.getenv(
        "FIVES_ROOT_FOLDER", os.path.join(REPO_ROOT, "datasets", "fives")
    )
    json_path = os.path.join(root_path, "fives_splits.json")
    out_root = os.path.join(REPO_ROOT, "verify_visually", "fives")
    os.makedirs(out_root, exist_ok=True)

    for split in ("train", "val", "test"):
        ds = FIVES_Dataset(root_dir=root_path, splits_json=json_path, split=split)
    ...
    print("\nFIVES visual verification complete.")
