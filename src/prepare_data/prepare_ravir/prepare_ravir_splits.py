...
def create_dataset_splits(
    ...
    train_images = sorted(
        [
            os.path.join(train_images_path, f)
    ...
    for img in test_images:
        mask = (
            os.path.join(test_images_path, os.path.basename(img))
            if os.path.basename(img) in test_masks
            else None
        )
        ...
        "train": [
            {
                ...
                "target": mask.replace(root_folder, ""),
            }
            for img, mask in zip(train_images_split, train_masks_split)
        ],
        "val": [
            {
                "image": img.replace(root_folder, ""),
            ...
            for img, mask in zip(val_images_split, val_masks_split)
        ],
        "test": test_data,
    }
